...
impl Config {
    pub(super) fn validated_keybinds(&self) -> (Option<String>, KeyCombo, Vec<String>, Keybinds) {
        ...
        for source in [BindingSource::User, BindingSource::Default] {
            ...
            apply_action!(keybinds.agent_grid, agent_grid, source);
...
"#,
    ...
    #[test]
    fn agent_grid_defaults_to_prefix_a() {
        let kb = Config::default().keybinds();
        assert_eq!(
            binding_triggers(&kb.agent_grid),
            vec![BindingTrigger::Prefix((
                KeyCode::Char('a'),
                KeyModifiers::empty()
            ))]
